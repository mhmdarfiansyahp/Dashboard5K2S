...
class Riksa_m extends CI_Model
{
    private $_table = "riksa";
    ...
    public function rules()
    {
        return [
            ...
            ['field' => 'masa_laku',
            'label' => 'Jadwal Uji Riksa',
            'rules' => 'required']
        ];
    }

    public function getAll() {
        $query = "SELECT a.id, a.jadwal, a.masa_laku, a.status, b.id as id_mesin, b.id_eng, b.name FROM riksa A join machine B on a.id_mesin = b.id where a.flag=1;";
           $hasil = $this->db->query($query);
        // $query = $this->db->where('flag', 1)->get('riksa'); 
    ...
    public function getTersertifikasi() {
        $query = $this->db->where('status', 'Tersertifikasi')->get('riksa'); 
    ...
    {
        $this->db->where('id', $id);
        return $this->db->update('riksa', $data); 
    }
    ...
    {
        $this->db->where('id', $id);
        return $this->db->delete('riksa', $data);
    }
    ...
    public function getAllbyFilter($month) {
        ...
        $query = "SELECT a.id, a.jadwal, a.masa_laku, a.status, b.id_eng, b.name, b.brand, b.type,b.lokasi, b.rangeCapacity, b.remark FROM riksa A join machine B on a.id_mesin = b.id where a.flag=1 and a.jadwal BETWEEN '".$start."' AND '". $end."';";
           $hasil = $this->db->query($query);
